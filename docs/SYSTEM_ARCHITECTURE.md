...
* **Arsitektur Seksi Penutup Adaptif 100vh (`.site-footer` / `.closing-sec`) & Fallback Label:**
  ...
  - **Standarisasi Smart Auto-Hide Navigasi Dock & Kontrol Audio Mengambang:**
    ...
    - **Ultra-Clean Outro Auto-Hide:** Saat tamu tiba di seksi penutup / outro footer paling bawah (`isNearBottom`), dock navigasi (`.bottom-dock`) dan kontrol mengambang (`.music-fab` / `.btn-top-menu`) otomatis tersembunyi (*autohide*) secara mutlak agar tampilan outro 100vh bebas gangguan visual dan bersih total. Seluruh kontrol akan meluncur masuk kembali (*reveal*) secara instan begitu tamu melakukan gestur scroll ke atas (`delta < -SCROLL_THRESHOLD`).
