...
2. **Master Catalog Template Engine (28 Tema Fisik):**
   - Setiap tema merupakan template HTML/CSS/JS mandiri di direktori `themes/{kategori}/{nama}.html` (Modern, Tradisional, Minimalis, dsb.). Termasuk rumpun tema Sulawesi Selatan: **Toraja** (`toraja.html`), **Toraja Rantepao** (`rantepao.html`), **Toraja Makale** (`makale.html`), **Bugis** (`bugis.html`), **Bone** (`bone.html`), **Wajo** (`wajo.html`), **Soppeng** (`soppeng.html`), **Makassar** (`makassar.html`), **Gowa** (`gowa.html`), **Maros** (`maros.html`), **Takalar** (`takalar.html`), dan **Bulukumba** (`bulukumba.html`) yang mengangkat kekayaan kultural dengan ornamen otentik dan 9 slot media lengkap.
...
7. **Smart Audit Protocol (Zero Data Bolong)**:
   ...
   - **Sinkronisasi Navigasi Runtime, Smart Dock Home Zone Guard & Ultra-Clean Outro Auto-Hide:** Seksi yang dimatikan otomatis terhapus dari DOM dan item navigasi dock bawah (`.bottom-dock a`) serta tombol audio floating (`.music-fab`) disembunyikan secara dinamis via `syncActiveTogglesUI()`. Selain itu, script mengawal status dock (`lux-at-home-zone`), memastikan dock bawah tetap tersembunyi secara murni di seksi Home/Hero dan hanya muncul saat scroll-up di seksi berikutnya. Saat tamu tiba di seksi penutup / footer paling bawah (`isNearBottom`), dock dan tombol kontrol otomatis tersembunyi (*outro auto-hide*) untuk tampilan 100vh yang bersih total, dan kembali muncul saat scroll-up.
...
4. **Pemisahan Desain & Operasional Galeri Kenangan Tamu**:
   ...
   - **Fitur Kamera Disposable Retro & Galeri Masonry Roll Stack (Opsi B):**
     ...
     - **Formula Kuota Acara, Invarian Anti-Hangus, & Smart Quota Boundary Guard:**
        ...
        - *Boundary Clamping Tamu Terakhir:* Ketika sisa pool foto lebih sedikit dari jatah roll (misal tersisa 8 foto dengan setting 15 roll), sistem secara otomatis menerapkan `effectiveShotsQuota = Math.min(configuredShotsQuota, remainingPool)` sehingga tamu terakhir dapat menghabiskan kuota pool secara presisi tanpa error 403. Saat pool penuh 100%, tamu disajikan pesan metafora analog hangat (*"Terima kasih banyak atas momen indahnya! Roll kamera kenangan untuk acara ini telah terisi penuh dengan cinta. Semua foto sedang kami proses dan simpan dengan aman ke dalam album kenangan pengantin ✨"*) tanpa mengekspos angka kuota atau kesan batasan paket demi menjaga wibawa pengantin.
        - *Sistem Peringatan Ambang Batas 80% (Outbound Email & Dashboard Banner):* Saat total foto mencapai $\ge 80\%$, backend secara asinkron menembakkan email notifikasi sekali (*idempotent*) ke pengantin dengan tautan langsung menuju Dasbor Momen privat (`/dashboard/moments`), dan dasbor momen klien memunculkan banner peringatan *Amber Gold* dengan tombol cepat *Top-Up +100 Foto via QRIS*.
...
7. **Proteksi Studio Editor Pasca Publish, Buka Kunci Darurat, & Atomic Single Deploy:**
   ...
   - **Atomic Single Deploy & Auto-Lock (`DEPLOY_AND_LOCK`):** Di puncak formulir saat mode perbaikan aktif tersedia tombol aksi **"Update Publikasi & Kunci Kembali"** yang mengeksekusi 1 kali kompilasi HTML penuh, migrasi/sinkronisasi ke Cloudflare R2, eksekusi pembersihan (*purge*) edge cache Cloudflare secara atomik untuk URL spesifik undangan terkait (`subdomain`, `slug`, `customDomain`), dan seketika mengunci kembali studio secara otomatis.
...
8. **Proteksi Siklus Download Galeri Tamu (ZIP), Layar Opening Editorial (`/sharemoment`), & Pemisahan Studio Fisik vs Digital:**
   ...
   - **Notifikasi Multi-Milestone Kuota Roll Dinamis & Zero-Embarrassment Copy:** Ambang batas peringatan kuota roll tidak di-hardcode. Dikonfigurasi dinamis oleh admin melalui setting `memories_notify_milestones` (default `"50,80,100"` atau nilai persentase kustom). Email peringatan terpicu otomatis secara non-blocking dengan template adaptif (50% Separuh Roll, 80% Antusiasme Tinggi, 100% Roll Penuh) dan mengarahkan tombol aksi langsung ke Dasbor privat (`/dashboard/moments`) untuk top-up yang aman. Saat roll penuh (100%), tamu menerima respon metafora analog yang sopan (*"Roll kamera kenangan telah terisi penuh dengan cinta..."*) untuk menjaga martabat pengantin, serta milestone di-rearm otomatis jika pengantin memperluas roll kuota.
   ...
   - **Integritas Konkurensi Database & Anti-Race Condition Multi-Worker:** Menutup celah konkurensi pada lingkungan multi-proses (PM2 cluster) melalui integrasi *PostgreSQL Advisory Transaction Locks* (`pg_advisory_xact_lock`) pada pencatatan RSVP dan unggah foto kenangan tamu (menjamin plafon kuota foto dan batas pax katering terlindungi anti-bocor secara matematis), *Atomic Compare-and-Swap* (`updateMany`) pada mutasi check-in meja resepsionis (mencegah duplikasi event souvenir), serta ekstraksi IP terpercaya via `getClientIp` dari reverse proxy (Cloudflare/Caddy/Nginx).
...
12. **Universal Vertical Glowing Luxury Timeline Standard (Love Story / Our Journey):**
    - Menstandarisasikan tampilan seksi Kisah Cinta di seluruh 19 tema master (Valente, Kalandra, Aurelia, Artisan, Papercut, Ameera, Mayang, Candani, Lagaligo, Toraja, Bugis, Makassar, dll.) dan Engine bawaan (`lib/themeEngine.ts` & `lib/demoRegistry.ts`).
...
1. **Master Tema Tradisional Toraja (`themes/traditional/toraja.html`):**
   ...
   - **Ornamen Budaya Otentik Toraja (`public/assets/ornaments/toraja/`):**
    ...
    - **Tipografi Etnik Berketerbacaan Tinggi:**
      * Judul Utama & Monogram: `Cinzel` (klasik sakral dan megah).
      * Kaligrafi & Aksen: `Great Vibes` (romantis dan anggun).
      * Teks Informasi & UI: `Plus Jakarta Sans` (keterbacaan optimal pada layar ponsel).
    ...
    - **Kepatuhan Blueprint 460px & Standarisasi 9 Slot Media Penuh:**
      * Panel undangan 460px di desktop widescreen dengan sidebar kiri hero jernih (tanpa scrim pekat).
...
1. **Master Tema Tradisional Toraja Makale (`themes/traditional/makale.html`):**
   ...
   - **Integrasi 9 Slot Media Lengkap (100% Invarian Media):**
     ...
     * `LANDING_COVER_DESKTOP`: Cover desktop widescreen landscape 16:9 fullscreen override (`/demo/makale/cover_desktop.webp`).
     * `HOME_PHOTO`: Foto hero pembuka kedua mempelai (`/demo/makale/home.webp`).
     * `DESKTOP_SIDEBAR`: Foto latar panggung desktop fixed kiri (`/demo/makale/hero.webp`).
     * `GLOBAL_FIXED_BG`: Kanvas latar belakang tetap (`/demo/makale/background.webp`).
     * `GROOM_PHOTO` & `BRIDE_PHOTO`: Foto potret individual masing-masing mempelai (`groom.webp` & `bride.webp`).
     * `GALLERY`: Grid galeri foto responsif 8 foto (`gallery_01.webp` .. `gallery_08.webp`) dengan fitur lightbox zoom.
     * `CLOSING_COVER`: Foto penutup seksi outro layar penuh 100vh adaptif (`/demo/makale/footer.webp`) dengan class dinamis `.site-footer.has-closing-photo`.
...
## 27. Spesifikasi Master Tema Daerah Sulawesi Selatan (v5.9.4)

1. **Penyempurnaan Master Base Bugis & Makassar:**
   - Menambahkan bingkai potret pembuka mempelai (*Home Arch Photo Frame*) `{{homePhotoUrl}}` pada seksi `#home` di `bugis.html` dan `makassar.html`.
   - Mengalihkan sumber aset demo ke folder terisolasi resmi `public/demo/bugis/` dan `public/demo/makassar/` dengan 9 slot media lengkap.
