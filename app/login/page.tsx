...
export default function ClientLoginPage() {
  ...
  const handleClientLogin = async (customEmail?: string) => {
    setLoading(true);
    setError(null);
    const clientEmail = customEmail || email || "client@luxenary.id";
    ...
    try {
      ...
      console.error(e);
      setError("Gagal masuk ke studio. Silakan refresh halaman dan coba kembali.");
      setLoading(false);
    }
  };
    ...
    <div className="min-h-screen flex items-center justify-center bg-[#faf7f2] px-4 py-12 relative overflow-hidden font-sans">
      ...
      <div className="max-w-md w-full bg-white border border-amber-900/10 rounded-3xl p-8 sm:p-10 shadow-xl relative z-10 text-stone-900 space-y-6">
        ...
        <div className="text-center space-y-1.5">
          ...
          <p className="text-xs text-stone-500">Kelola desain, multi-acara, tamu, dan foto pre-wedding Anda</p>
        </div>
        ...
        {/* 1-Click Fast Client Access (No Google Key Required) */}
        <div className="p-4 bg-amber-50/60 border border-amber-200/80 rounded-2xl space-y-2">
          <div className="flex items-center justify-between">
            <span className="text-[11px] font-bold text-amber-900 uppercase tracking-wider">Akses Langsung Studio Klien:</span>
            <span className="text-[10px] text-amber-700 font-semibold">Didan &amp; Nasha</span>
          </div>
          <button
            type="button"
            onClick={() => handleClientLogin("client@luxenary.id")}
            disabled={loading}
        ...
        <div className="relative flex items-center justify-center">
          ...
          <span className="bg-white px-3 text-[11px] text-stone-400 font-bold uppercase tracking-wider absolute">atau</span>
        </div>
        ...
        >
          <div>
            <label className="block text-[11px] font-bold text-stone-700 mb-1">Email Klien / Pengantin</label>
            <input
              type="email"
              value={email}
              onChange={(e) => setEmail(e.target.value)}
              placeholder="nama@email.com atau client@luxenary.id"
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full py-3 bg-stone-900 hover:bg-stone-800 text-white font-semibold rounded-xl text-xs transition disabled:opacity-50 cursor-pointer"
          >
        ...
        <button
          ...
          className="w-full py-3 bg-stone-50 hover:bg-stone-100 border border-stone-200 rounded-xl flex items-center justify-center gap-2.5 text-xs font-bold text-stone-800 transition cursor-pointer"
        >
          ...
          <span>Login dengan Google OAuth</span>
        </button>
        ...
        <div className="flex items-center justify-between pt-2 text-[11px] text-stone-500 border-t border-stone-100">
          <a href="/demo" className="hover:text-amber-700 transition font-semibold">
            ← Showroom Tema Demo
          </a>
